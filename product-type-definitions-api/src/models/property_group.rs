/* 
 * Selling Partner API for Product Type Definitions
 *
 * The Selling Partner API for Product Type Definitions provides programmatic access to attribute and data requirements for product types in the Amazon catalog. Use this API to return the JSON Schema for a product type that you can then use with other Selling Partner APIs, such as the Selling Partner API for Listings Items, the Selling Partner API for Catalog Items, and the Selling Partner API for Feeds (for JSON-based listing feeds).
 *
 * OpenAPI spec version: 2020-09-01
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

/// PropertyGroup : A property group represents a logical grouping of schema properties that can be used for display or informational purposes.

#[allow(unused_imports)]
use serde_json::Value;

#[derive(Debug, Serialize, Deserialize)]
pub struct PropertyGroup {
  /// The display label of the property group.
  #[serde(rename = "title")]
  title: Option<String>,
  /// The description of the property group.
  #[serde(rename = "description")]
  description: Option<String>,
  /// The names of the schema properties for the property group.
  #[serde(rename = "propertyNames")]
  property_names: Option<Vec<String>>
}

impl PropertyGroup {
  /// A property group represents a logical grouping of schema properties that can be used for display or informational purposes.
  pub fn new() -> PropertyGroup {
    PropertyGroup {
      title: None,
      description: None,
      property_names: None
    }
  }

  pub fn set_title(&mut self, title: String) {
    self.title = Some(title);
  }

  pub fn with_title(mut self, title: String) -> PropertyGroup {
    self.title = Some(title);
    self
  }

  pub fn title(&self) -> Option<&String> {
    self.title.as_ref()
  }

  pub fn reset_title(&mut self) {
    self.title = None;
  }

  pub fn set_description(&mut self, description: String) {
    self.description = Some(description);
  }

  pub fn with_description(mut self, description: String) -> PropertyGroup {
    self.description = Some(description);
    self
  }

  pub fn description(&self) -> Option<&String> {
    self.description.as_ref()
  }

  pub fn reset_description(&mut self) {
    self.description = None;
  }

  pub fn set_property_names(&mut self, property_names: Vec<String>) {
    self.property_names = Some(property_names);
  }

  pub fn with_property_names(mut self, property_names: Vec<String>) -> PropertyGroup {
    self.property_names = Some(property_names);
    self
  }

  pub fn property_names(&self) -> Option<&Vec<String>> {
    self.property_names.as_ref()
  }

  pub fn reset_property_names(&mut self) {
    self.property_names = None;
  }

}



